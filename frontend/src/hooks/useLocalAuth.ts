...
export function useLocalAuth() {
  ...
  const storedUserToProfile = useCallback((user: StoredUser): UserProfile => ({
  ...
  const profileToStoredUser = useCallback((profile: UserProfile, password: string): StoredUser => ({
  ...
  const updateProfile = useCallback((updates: Partial<UserProfile>) => {
    ...
    const users = getStoredUsers();
    const userIndex = users.findIndex((u) => u.email === authState.userProfile!.email);
    ...
    if (userIndex !== -1) {
      ...
      users[userIndex] = profileToStoredUser(updatedProfile, currentUser.password);
      saveUsers(users);
