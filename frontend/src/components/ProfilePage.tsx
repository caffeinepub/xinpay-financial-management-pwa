import { UserProfile, BankDetailsWithoutNetBankingV1 } from '../backend';
import { useLocalAuth } from '../hooks/useLocalAuth';
import { Card } from '@/components/ui/card';
import { Button } from '@/components/ui/button';
import { User, Mail, Wallet, LogOut, Shield, RefreshCw, Building2, CreditCard, Loader2, Clock } from 'lucide-react';
import { useIsCallerAdmin, useSetAllInrBalancesToDefault, useGetAllCompleteBankDetails } from '../hooks/useQueries';
    ...
    <div className="p-4 bg-[#0B1C14] rounded-xl border border-[#00E5FF]/10 space-y-3">
      ...
      <div className="flex items-start justify-between gap-2">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <div className="min-w-0">
            <p className="text-white text-sm font-semibold truncate">{details.bankName}</p>
            <p className="text-gray-500 text-xs font-mono truncate">{shortPrincipal}</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 gap-2 text-xs">
        <div className="p-2 bg-[#1C2431] rounded-lg">
          ...
          <p className="text-white font-mono mt-0.5">{maskAccountNumber(details.accountNumber)}</p>
        </div>
        <div className="p-2 bg-[#1C2431] rounded-lg">
        ...
        {details.emailId && (
          <div className="p-2 bg-[#1C2431] rounded-lg">
            ...
            <p className="text-white font-mono mt-0.5 truncate">{details.emailId}</p>
          </div>
        )}
      </div>
      ...
      <div className="flex items-center gap-2 p-2 bg-[#1C2431] rounded-lg text-xs">
        ...
        <span className="text-white font-mono">••••{details.cardNumber.slice(-4)}</span>
...
export default function ProfilePage({ userProfile }: ProfilePageProps) {
  ...
  const { data: allBankDetails, isLoading: bankDetailsLoading } = useGetAllCompleteBankDetails();
  const [lastResult, setLastResult] = useState<{ updatedCount: number; defaultBalance: number } | null>(null);
  ...
  const handleSetAllBalances = async () => {
    if (confirm('Are you sure you want to set all users\' INR balance to ₹1000.00? This action cannot be undone.')) {
      try {
  ...
  return (
    <div className="min-h-screen bg-[#0B1C14] space-y-4">
          ...
          <div className="flex items-center justify-between p-3 bg-[#0B1C14] rounded-lg">
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-sm font-medium text-blue-400">₹{inrBalance.toFixed(2)}</p>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center justify-between p-3 bg-[#0B1C14] rounded-lg">
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-sm font-medium text-green-400">${usdtBalance.toFixed(2)}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <>
          ...
          <Card className="bg-[#1C2431] border-orange-500/30 p-6 space-y-4">
            <div className="flex items-center space-x-2">
              ...
              <h3 className="text-lg font-bold text-orange-400">Admin Controls</h3>
            </div>
          ...
          <Card className="bg-[#1C2431] border-orange-500/30 p-6 space-y-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-2">
                ...
                <h3 className="text-lg font-bold text-orange-400">Bank Account Verification</h3>
              </div>
              {allBankDetails && (
                <span className="text-xs text-gray-400 bg-[#0B1C14] px-2 py-1 rounded-full">
                  {allBankDetails.length} account{allBankDetails.length !== 1 ? 's' : ''}
                </span>
              )}
            </div>
            ...
            {bankDetailsLoading ? (
              ...
              <div className="text-center py-6">
                ...
                <p className="text-gray-400 text-sm">No bank accounts submitted yet</p>
              </div>
            ) : (
              <div className="space-y-3">
      ...
      <footer className="text-center space-y-1 pt-4">
        <p className="text-sm text-gray-400">© {new Date().getFullYear()} XinPay | Trusted Payment Platform</p>
