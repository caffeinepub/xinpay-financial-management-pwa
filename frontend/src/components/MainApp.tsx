...
import { Home, Wallet, Users, Headphones, User, Share2, Landmark } from 'lucide-react';
...
type ActiveTab = 'home' | 'deposit' | 'refer' | 'support' | 'profile' | 'accounts';

export default function MainApp({ userProfile: localUserProfile }: MainAppProps) {
  const [activeTab, setActiveTab] = useState<ActiveTab>('home');
  const [showShareModal, setShowShareModal] = useState(false);
  const { updateProfile } = useLocalAuth();

  // Fetch backend profile - always fetch fresh data with zero cache
  const { data: backendProfile, isLoading: profileLoading, refetch } = useGetCallerUserProfile();
  ...
  const shareViaTelegram = () => {
    const appLink = window.location.origin;
    const message = `Check out XinPay - Professional Financial Management Platform`;
    window.open(`https://t.me/share/url?url=${encodeURIComponent(appLink)}&text=${encodeURIComponent(message)}`, '_blank');
  };
  ...
  const navItems: { tab: ActiveTab; icon: React.ReactNode; label: string }[] = [
    ...
    { tab: 'accounts', icon: <Landmark className="h-5 w-5" />, label: 'Accounts' },
    { tab: 'refer', icon: <Users className="h-5 w-5" />, label: 'Refer' },
    { tab: 'support', icon: <Headphones className="h-5 w-5" />, label: 'Support' },
  ...
  return (
    <div className="min-h-screen bg-[#0B1C14] pb-20">
      ...
      <main className="px-4 py-6">
        {activeTab === 'home' && <HomePage userProfile={displayProfile} onNavigate={handleNavigate} />}
        {activeTab === 'deposit' && <DepositPage />}
        {activeTab === 'accounts' && <ActiveAccountsPage />}
        {activeTab === 'refer' && <ReferPage />}
        {activeTab === 'support' && <SupportPage />}
        {activeTab === 'profile' && <ProfilePage userProfile={displayProfile} />}
      </main>
